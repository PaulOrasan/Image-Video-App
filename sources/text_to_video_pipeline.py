...
class TextToVideoPipeline(StableDiffusionPipeline):
    ...
    def encode_latents(self, image: PIL.Image):
        img = np.array(image.resize((512,512)).convert("RGB"))
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if start_image is not None:
            xT = self.encode_latents(start_image)
            shape = (batch_size, num_channels_latents, 1, height //
                     self.vae_scale_factor, width // self.vae_scale_factor)
            xT = self.DDPM_forward(xT, 0, 1000, device=device, shape=shape, text_embeddings=text_embeddings,
                                   generator=generator)
